...
def multi_label_metric(y_gt, y_pred, y_prob):
    # return 0,0
    y_gt = torch.concat(y_gt).reshape(-1,132).cpu().detach().numpy()
    y_pred = torch.cat(y_pred).cpu().reshape(-1,132).detach().numpy()
    y_prob = torch.cat(y_prob).cpu().reshape(-1,132).detach().numpy()
